...
The session's deck list is therefore one record per deck — `deckList: { id, emoji }[]` — and not a list of ids: the list keeps order and membership as before (0029), while the emoji rides beside the id rather than inside `SessionDeck`, which a clip copies and would otherwise carry a decoration it can never apply. The stored emoji is validated as durable text, not as pool membership: which emoji exist is the interface's business, and the stored shape refuses only what no yard could have been given. Sessions written before this shape do not validate and are discarded (0026).
