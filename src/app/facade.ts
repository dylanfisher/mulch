...
// Re-exported where the console and its suite already read it from: the counters are the facade's
// to answer for, and only their declaration moved (0045).
export { HEAP_READ_INTERVAL_MS, type Stats };

/**
 * The whole session as JSON, with `at`: how long this performance has been running, which the
 * global Stop returns to nought (0315). Not the audio clock — that is `stats().at`, which is what
 * an envelope's own `at` is stated on.
 */
...
export function createInstrument(
  ...
  /**
   * Where this performance's elapsed run is measured from. The audio clock cannot be rewound — it
   * is the context's own `currentTime`, every envelope is stamped against it and `stats().at`
   * still reads it — so the run a hand hears is that clock less this, and the global Stop moves
   * this rather than the clock (0315). `probe().at` is the only reading it moves, which is the
   * one that means "how long has this been performing": where an export's take begins. Nought to
   * begin with, which is the clock itself.
   */
  let runFrom = 0;
  const rewind = (): void => {
    runFrom = clock.now();
  ...
  return {
    ...
    probe: () => ({ at: clock.now() - runFrom, ...store.getState() }),
    stats: () => {
