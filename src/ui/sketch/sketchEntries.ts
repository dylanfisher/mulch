...
export const SKETCH_DRIFTS: readonly SketchEntry[] = [
  ...
  },
  {
    ...
    built:
      "aimFeedback in src/ui/moireCanvasFeedback.ts already scales the ghost by FEEDBACK_ZOOM.value at three percent and turns it a fraction; the zoom goes to the dial's band and the centre moves with the wind's veer. Frame-side entirely — one drawImage transform, no bake, no key — and feedbackAlpha's ceiling is what keeps the sum from blowing out, which the closed form in sketchField.ts states as the geometric series it is.",
