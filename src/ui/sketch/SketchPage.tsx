 ...
 *   The surface the ground eight argue with → src/ui/PlayerCard.tsx. The picture the drift thirteen
...
function DriftIntro() {
  return (
    <p className="max-w-3xl type-body text-muted-foreground">
      ...
      is a bake and a frame is a fill — so the thirteen are a plan&apos;s worth of parts and not
      thirteen wishes. The stand-in is two gratings a pixel apart in pitch, which is what the real
      picture is under everything else; what is judged is the move, not the weave. The pictures are
      painted by a pixel loop on this bench and would not be in the painter, where anything per
      frame is a transform or a slice and everything else is a stepped key (0129, 0144).
    </p>
  );
}
