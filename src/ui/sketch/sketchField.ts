 ...
 *   domain warp, a smooth minimum, a terrace — each written once so the pictures cannot
 ...
 *   writes one → src/ui/sketch/SketchDriftStage.tsx. The ramp a field is read through, which the
 *   painter took (0301) → src/lib/moireColour.ts. The real picture's own arithmetic, which
...
export function warp(x: number, y: number, amount: number, phase = 0): [number, number] {
  ...
  const x1 = x + amount * Math.sin(TAU * (WARP_DOWN.value * y + phase));
  const y1 = y + amount * Math.sin(TAU * (WARP_ACROSS.value * x1 - phase));
