...
/** The corner slot as this file reads one: what it wears, and the endings that stop at it. */
type Fence = (event: { stopPropagation: () => void }) => void;
type Corner = {
  "data-slot"?: string;
  children: unknown;
  onPointerDown: Fence;
  onPointerUp: Fence;
  onPointerCancel: Fence;
  onLostPointerCapture: Fence;
  onKeyUp: Fence;
};

describe("ParameterKnob corner slot", () => {
  it("draws a corner only for a knob handed one to wear", () => {
    const instrument = createInstrument(manualClock());
    const knob = (corner?: ReactTypes.ReactNode) => {
      refs = [];
      refIndex = 0;
      const rendered = ParameterKnob({
        instrument,
      ...
      if (!isValidElement<{ children: unknown[] }>(rendered)) throw new Error("no wrapper");
      return rendered.props.children[2];
    };

    // The slot is the knob's and what fills it is the caller's: handed a control it wears one in
    // its corner, handed nothing it draws no box there at all.
    const worn = knob(<i data-badge="true" />);
    if (!isValidElement<Corner>(worn)) throw new Error("no corner");
    expect(worn.props["data-slot"]).toBe("knob-corner");
    expect(worn.props.children).toEqual(<i data-badge="true" />);
    expect(knob()).toBeNull();

    // And the corner is a fence: every ending the wrapper watches for stops at it, so a press on
    // what it wears — or a drag on a slider inside the window that press opens, which portals back
    // along this tree — is not a gesture on the dial it sits over.
    const { onPointerDown, onPointerUp, onPointerCancel, onLostPointerCapture, onKeyUp } =
      worn.props;
    for (const ending of [
      onPointerDown,
      onPointerUp,
      ...
      const stopped = vi.fn(() => {});
      ending({ stopPropagation: stopped });
