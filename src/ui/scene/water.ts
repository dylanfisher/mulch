 ...
 *   src/lib/moireScene.ts, which is where the shortest signed offset the blades are reached
 *   through lives (`sceneNear`). Which names read as this one → src/lib/yardScene.ts. The hash the
...
/**
 * How wide the slow diagonal swell that bends the ripples and lights them in bands is. Every pitch
 * this ground reads — this one, the two the glints beat out of and the dash's — is multiplied by
 * how close the frame stands before it is snapped (`terms.reach`, 0335), so the water is coarser
 * close up. The blades below are not marks and keep the size they were written at.
 */
...
export const water: Scene = {
  ...
  ground: (x, y, terms) => {
    const ripple = RIPPLE.value * terms.reach;
    const swell = SWELL.value * terms.reach;
    // The swell runs diagonally and comes round on both axes, twice as wide across as it is deep.
    const swellAcross = sceneRepeat(terms.width, 2 * swell);
    const swellDown = sceneRepeat(terms.height, swell);
    const rolling = sceneAxis(x / swellAcross + y / swellDown);
    // The two pitches the glints beat out of, snapped onto the tile and bent together by the
    // swell: a bend and never a third lattice.
    const rippleDown = sceneRepeat(terms.height, ripple);
    const beatDown = sceneRepeat(terms.height, BEAT.value * terms.reach);
    const bent = y + BEND * rolling;
    ...
    // a hash does not.
    const rows = sceneCells(terms.height, ripple);
    const row = wrap(Math.round(bent / rippleDown), rows);
    const dashAcross = sceneRepeat(terms.width, DASH.value * terms.reach);
    const dashDown = sceneRepeat(terms.height, (DASH.value * terms.reach) / STEEP);
    ...
    const deep = DEEP.value + BAND * rolling;
    let value = deep + lit * (0.4 + 0.6 * rolling) * (0.99 - deep);
    ...
    for (const reed of BLADES) {
      const dx = sceneNear(x - reed.x, fieldAcross);
      const dy = sceneNear(y - reed.y, fieldDown);
