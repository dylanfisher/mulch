 ...
 *   nothing. A folded yard draws it in its header, where the slack is. Switched off at the header
 *   (src/ui/driftShown.ts) none of it is mounted at all, on any yard.
 * @instead The switch itself, and where the choice is kept → src/ui/driftShown.ts. The zoom,
 *   the window and the press that chooses between them → src/ui/driftZoom.tsx. What the rows
 *   are made of, and the window they are drawn across → src/ui/moireRows.ts.
...
import { driftTitle, MOIRE_STRIP, RECURRENCE_TOOLTIP, yardLabel } from "@/lib/copy";
import { MOIRE_POP_OUT, MOIRE_POP_OUT_TOOLTIP } from "@/lib/copyDrift";
...
function useMoireRows(
  ...
  const refill = useCallback(() => {
    ...
    if (from.current !== session) {
      ...
      carryArrivals(painted.current, session);
...
/**
 * The switch, and nothing under it. Switched off the picture is not drawn cheaply or at a lower
 * cadence: it is not there — no canvas mounted, no surface asked for, no tile baked, no frame
 * callback and no window — which is the same way a closed overlay costs nothing (0070, 0397).
 * The read is a hook and so runs first; everything the picture needs is in the body below, which
 * is mounted only where the switch says so.
 */
export function MoireStrip(props: MoireProps & { className?: string }) {
