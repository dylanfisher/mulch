 ...
 *   → src/ui/moireScreen.ts. The frame before this one, kept and laid back in →
 *   src/ui/moireCanvasFeedback.ts, which this splits that one pass out into. What a row is,
...
import { feedFrame, forget } from "@/ui/moireCanvasFeedback";
...
export function paintMoire(
  ...
  // Nothing is laid where this canvas's first screen is still baking: the ink would go down flat,
  // which is the whole picture in one solid colour rather than a picture — the flash a popped-out
  // window opened on (0384). The bake asks this painting for another when it lands, exactly as a
  // late row tile does (0144), so the picture arrives on the grid and never before it.
  if (
    !inkThrough(
      canvas,
      context,
    ...
    // Cleared already at the top of this painting and nothing has touched the canvas since: what
    // the window shows is its own background until the tile lands.
    forget(canvas);
    endPainting();
    return;
  }
