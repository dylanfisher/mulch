 ...
 */
// One case per thing the header says of a load — its name, the id it withholds, the length it no
// longer asks for, the frequency and where it stands — and each is a fact of one control, so a
// split would part the header's readout from the header's field. See
...
describe("the source menu", () => {
  ...
  // P18: every format is the same import, so the picker is offered exactly the shared list; the
  // bytes' own path to the store is src/ui/DeckImport.test.tsx's.
