...
/**
 * And how hard the field was jolting, with what it was last jolted by, which landings were
 * still pushing the marks and which of their sparks were still flashing. The same argument the wind's
...
export function carryJolt(from: MoireRowSet, to: MoireRowSet): void {
  ...
  const { pushes, sparks, ...struck } = from.jolt;
  Object.assign(to.jolt, struck);
  to.jolt.pushes.forEach((push, at) => {
    const was = pushes[at];
    if (was !== undefined) Object.assign(push, was);
  });
  // And the sparks of that same landing, written into the fresh set's own slots for the pushes'
  // reason: a flash mid-fall outlives an effect being added, and two sets stepping one list do not.
  to.jolt.sparks.forEach((spark, at) => {
    const was = sparks[at];
    if (was !== undefined) Object.assign(spark, was);
  });
}
