 ...
 *   of the part standing and the character it is drawn as, written once a frame straight into the
 *   element (0157). Per-frame and nothing else — no command, nothing durable, no React state
...
export function PlayerStanding({
  ...
  const paint = useCallback(() => {
    ...
    const text =
      part === undefined
        ? ""
        : `${PLAYER_STANDING_LABEL} ${partBadge(part.id)} ${PLAYER_CHARACTER_LABELS[part.character]}`;
