...
export type PlayerSpec = BedSpec &
  ...
  TravelSpec & {
    ...
    varyChance: number;
    /**
     * How the pattern is arranged by hand: the songs it walks in turn, each a run of parts — or
     * none at all, which is every jump drawn under the numbers above
     * (0153, P170). A list rather than a count, because the order is the arrangement. Held
     * whatever `arrange` says and walked only while `arrange` is zero: which author is live is a
     * rule and not a second field, so a hand's songs survive a spell of drawing untouched (0158).
     */
