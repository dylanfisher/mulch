...
export const lanePreview = async ({ page }) => {
  ...
  const cutoffMark = page.getByLabel("Yard A EQ/Filter 1 Freq Automation");
  await cutoffMark.scrollIntoViewIfNeeded();
  await cutoffMark.hover();
  await page.getByLabel(/^Yard A EQ\/Filter 1 Freq Lane, \d+ points$/u).waitFor();
  ...
  const sample = () =>
    page.evaluate(() => {
      const indicator = document.querySelector(
        '[aria-label="EQ/Filter 1"] [role="slider"][aria-label="Freq"] line',
      );
