...
export const formats = async ({ page, root }) => {
  ...
  await page.locator('input[aria-label="Import audio for deck b"]').setInputFiles({
    name: "notes.txt",
    mimeType: "text/plain",
    buffer: Buffer.from("not audio"),
  });
  const refusal = await page.locator('section[aria-label^="Deck b"] [role="alert"]').textContent();
  ...
  const nonWav = await page.evaluate(async (bytes) => {
    ...
    return {
      ...
      accept: document
        .querySelector('input[aria-label="Import audio for deck b"]')
        .getAttribute("accept"),
    };
  }, flac);
