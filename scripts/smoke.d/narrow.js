...
export const narrowShell = async ({ page }) => {
  ...
  const narrow = await page.evaluate((slack) => {
    ...
    return {
      ...
      clipped,
      // One waveform per deck by construction, anchored at both ends. `section[aria-label^="Deck"]`
      // would also match each deck's effect rack, labelled `Deck <id> effects`
      ...
      decks: document.querySelectorAll('canvas[aria-label^="Deck "][aria-label$=" waveform"]')
        .length,
